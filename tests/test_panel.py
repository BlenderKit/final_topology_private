...
record, _ = draw_for("INCLINATION_LIMIT", lambda ac: setattr(ac, "inclination_axis", "CUSTOM"))
note("inclination_axis_vector" in record and "inclination_max_angle" in record, "inclination with a custom axis shows the vector")
record, _ = draw_for("ARC")
note("stop_at_poles" not in record and "arc_same_angle" in record, "arc has no pole/turn stops")
record, _ = draw_for("CIRCLE")
