...
def set_modifiers_start(obj):
    ...
    for modifier in obj.modifiers:
        ...
        if modifier.type == "SUBSURF":
            ...
            modifier.levels =  max(1,min(2, modifier.levels))

        # If geometry node modifier with the group "Final Topology extended subsurf" is present, use it instead of subsurf
        if modifier.type == "NODES" and modifier.node_group.name == "Final Topology extended subsurf":
            modifier.show_viewport = True
            modifier.show_in_editmode = True
            mod_settings["levels"] = modifier["Socket_2"]
            modifier["Socket_2"] = min(2, modifier["Socket_2"])
...
def get_subdivision_modifier_level(obj):
    ...
    if obj is not None and obj.type == "MESH":
        for modifier in obj.modifiers:
            ...
            # check the special FT modifier with gn group named "Final Topology extended subsurf" present
            if modifier.type == "NODES" and modifier.node_group.name == "Final Topology extended subsurf":
                if modifier.show_in_editmode and modifier.show_viewport:
                    return modifier["Socket_2"]
    return None
...
def final_topology_optimization_step(self, context, iterations=1, neighbours=1):
    ...
    if s_levels is None:
        self.report(
            {"WARNING"},
            "Only objects with subdivision modifier or FTSub modifier are supported by now.",
        )
        return False
